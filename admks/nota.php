		...
		if ($tcc != 0) //jika iya
			...
			if ($qty_all >= $jml_cc1)
				...
				if ($dtx_toko > $qty_all)
					...
					{
					$s_gudang = $qty_all - $dtx_toko; //sisa utk gudang
					$s_toko =  $qty_all - $s_gudang; //sisa utk toko
			...
			if (empty($jmlx))
				...
				if ($jmlx >= $jml_cc1)
					...
					if ($dtx_toko > $jmlx)
						...
						{
						$s_gudang = $jmlx - $dtx_toko; //sisa utk gudang
						$s_toko =  $jmlx - $s_gudang; //sisa utk toko
					...
					//ke detail....
					//insert
					mysqli_query($koneksi, "INSERT INTO nota_detail(kd, kd_nota, kd_brg, qty, qty_toko, qty_gudang, subtotal, postdate) VALUES ".
									"('$x', '$notakd', '$brgkd', '$jmlx', '$s_toko', '$s_gudang', '$stotx', '$today')");
...
if (($_POST['s'] == "hapus")
	...
	$qty_toko = nosql($rcc['qty_toko']);
	$qty_gudang = nosql($rcc['qty_gudang']);

	//update stock kembali...
	mysqli_query($koneksi, "UPDATE stock ".
					"SET jml_toko = jml_toko + '$qty_toko', ".
					"jml_gudang = jml_gudang + '$qty_gudang' ".
					"WHERE kd_brg = '$kd_brg'");
...
<input name="layar" type="text" size="30" value="'.$tuh_total.'" class="layar" style="text-align:right;width:400;height:70" readonly>
<br>
</td>
</tr>
</table>

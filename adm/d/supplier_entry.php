...
//nilai
$filenya = "supplier_entry.php";
$diload = "document.formx.nama.focus();";
$judul = "Entry Data Supplier";
$judulku = $judul;
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($_POST['btnSMP'])
	...
	if ((empty($nama)) OR (empty($alamat)) OR (empty($telp)))
		...
		{ ///cek
		$qcc = mysqli_query($koneksi, "SELECT * FROM m_supplier ".
								"WHERE nama = '$nama'");
		...
		if ($tcc > 1)
			...
			//re-direct
			$pesan = "Ditemukan Duplikasi Supplier : $nama. Harap Segera Diperhatikan...!!";
			pekem($pesan,$filenya);
			exit();
			}
		else
			...
			if ($tcc != 0)
				...
				//re-direct
				$pesan = "Supplier : $nama, Sudah Ada. Harap Ganti Yang Lain...!!";
				pekem($pesan,$filenya);
				exit();
				}
			else
				{
				mysqli_query($koneksi, "INSERT INTO m_supplier(kd, nama, singkatan, alamat, telp, hp, fax, email, ".

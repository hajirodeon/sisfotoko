...
if ($_POST['btnSMP'])
	...
	for ($ongko=1;$ongko<=$limit;$ongko++)
		...
		$jml_gd = nosql($rcc['jml_gudang']);
		$jml_tk = nosql($rcc['jml_toko']);
		$jml_tot = $jml_tk + $jml_gd;


		//nek input toko terlalu besar. cuekin aja.
		//beda urusannya, jika lebih kecil
		if ($xtkx > $jml_tot)
			{
			//cuekin
			}
			...
			{
			//jml stock
			if (($jml_gd != $xgdx) OR ($jml_tk != $xtkx))
				{
				//sesuaikan
				mysql_query("UPDATE stock SET jml_gudang = '$xgdx', ".
								"jml_toko = '$xtkx' ".
								"WHERE kd_brg = '$xkdx'");
				}
			}
...
if ($_POST['btnHPS'])
	...
	for ($i=1;$i<=$limit;$i++)
		...
		//del --> m_brg
		mysql_query("DELETE FROM m_brg ".
						"WHERE kd = '$kd'");

		//del --> stock
		mysql_query("DELETE FROM stock ".
						"WHERE kd_brg = '$kd'");
		...
		//del --> stock_nostock
		mysql_query("DELETE FROM stock_nostock ".
						"WHERE kd_brg = '$kd'");
		...
		//del --> jual_detail
		mysql_query("DELETE FROM jual_detail ".
						"WHERE kd_brg = '$kd'");
...
//total semua barang
$qtotg = mysql_query("SELECT m_brg.*, stock.* ".
						"FROM m_brg, stock ".
						"WHERE stock.kd_brg = m_brg.kd");
$rtotg = mysql_fetch_assoc($qtotg);
$ttotg = mysql_num_rows($qtotg);
...
<input name="kunci" type="text" size="15" '.$x_enter2.'>
...
if ((!empty($katcari)) AND (!empty($kunci)))
	...
	<td width="50" align="center">
	<strong><font color="'.$warnatext.'">Stock. Gudang</font></strong>
	</td>
	...
	<td width="50" align="center">
	<strong><font color="'.$warnatext.'">Stock. Total</font></strong>
	</td>
	...
	if ($count != 0)
		...
		do
			...
			edit_window=dhtmlmodal.open(\'Edit Stock Barang\', \'iframe\', \'brg_entry_edit.php?s=edit&brgkd='.$brgkd.'\', \'Edit Stock Barang\', \'width=700px,height=190px,center=1,resize=0,scrolling=0\');
			edit_window.onClose=function()
				{
				location.href=\''.$filenya.'?katcari='.$katcari.'&kunci='.$kunci.'&page='.$page.'\';
			...
			<td>'.$satuan.'</td>
			<td align="center">
			<input name="tk'.$nomer.'" type="text" style="text-align:right" value="'.$jml_toko.'"
			...
			<td align="center">
			<input name="gd'.$nomer.'" type="text" style="text-align:right" value="'.$jml_gudang.'" size="5" class="input" readonly>
			</td>
			<td align="center">
			<input name="jmin'.$nomer.'" type="text" style="text-align:right" value="'.$jml_min.'" size="5" class="input" readonly>
			</td>
			<td align="center">
			<input name="jtot'.$nomer.'" type="text" style="text-align:right" value="'.$jml_total.'" size="5" class="input" readonly>
			</td>
			<td align="center">
			<input name="hb'.$nomer.'" type="text" style="text-align:right" value="'.$hrg_beli.'" size="10" class="input" readonly>
			</td>
			<td align="center">
			<input name="hj'.$nomer.'" type="text" style="text-align:right" value="'.$hrg_jual.'" size="10" class="input" readonly>
			</td>

...
if ($_POST['btnSMP'])
	...
	for($ongko=1;$ongko<=$limit;$ongko++)
		...
		if (($hrg_beli != $xhbx) OR ($hrg_jual != $xhjx))
			{
			//sesuaikan
			mysqli_query($koneksi, "UPDATE stock SET hrg_beli = '$xhbx', ".
							"hrg_jual = '$xhjx' ".
							"WHERE kd_brg = '$xkdx'");
			}
		}
...
if ((!empty($katcari)) AND (!empty($kunci)))
	...
	if ($count != 0)
		...
		do
			...
			$jml_tot = $jml_toko + $jml_gudang;
			$hrg_beli = nosql($data['hrg_beli']);
			$hrg_jual = nosql($data['hrg_jual']);

			//jml. rusak ////////////////////////////////////////////////////////////
			$qjrus = mysqli_query($koneksi, "SELECT SUM(jml) AS jrus FROM stock_rusak ".
									"WHERE kd_brg = '$kd'");
			$rjrus = mysqli_fetch_assoc($qjrus);
			...
			//jml. hilang ////////////////////////////////////////////////////////////
			$qjhil = mysqli_query($koneksi, "SELECT SUM(jml) AS jhil FROM stock_hilang ".
									"WHERE kd_brg = '$kd'");
			$rjhil = mysqli_fetch_assoc($qjhil);
			...
			<td align="center">
			<input name="gd'.$nomer.'" type="text" style="text-align:right" value="'.$jml_gudang.'"
			onKeyUp="document.formx.jtot'.$nomer.'.value=eval(document.formx.tk'.$nomer.'.value) + eval(document.formx.gd'.$nomer.'.value);"
			...
			onKeyDown="var keyCode = event.keyCode;
				if (keyCode == 38)
					{
					document.formx.gd'.$nil.'.focus();
					}

				if (keyCode == 40)
					{
					document.formx.gd'.$nild.'.focus();
					}

				if (keyCode == 37)
					{
					document.formx.tk'.$nomer.'.focus();
					}
			...
			<td align="center">
			<input name="jmin'.$nomer.'" type="text" style="text-align:right" value="'.$jml_min.'"
			onKeyPress="return numbersonly(this, event)"
			onKeyDown="var keyCode = event.keyCode;
				if (keyCode == 38)
					{
			...
			<td align="center">
			<input name="jtot'.$nomer.'" type="text" style="text-align:right" value="'.$jml_tot.'" size="5" class="input" readonly>
			</td>
			<td align="center">
			<input name="hb'.$nomer.'" type="text" style="text-align:right" value="'.$hrg_beli.'"
			onKeyDown="var keyCode = event.keyCode;
				if (keyCode == 38)
					{
			...
			<td align="center">
			<input name="hj'.$nomer.'" type="text" style="text-align:right" value="'.$hrg_jual.'"
			onKeyDown="var keyCode = event.keyCode;
				if (keyCode == 38)
					{

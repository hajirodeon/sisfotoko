...
if ($s == "edit")
	...
	$jml_gudang = nosql($rowx['jml_gudang']);
	...
	if ($s == "edit")
		...
		if ((empty($kode)) OR (empty($nama)))
			...
			if ($tcc > 1)
				...
				{
				//update m_brg
				mysql_query("UPDATE m_brg SET kd_merk = '$merkkd', ".
								"kd_kategori = '$katkd', ".
								"kd_satuan = '$stkd', ".
								"kode = '$kode', ".
								"barkode = '$barkode', ".
				...
				//update stock
				mysql_query("UPDATE stock SET jml_gudang = '$jml_gudang', ".
								"jml_toko = '$jml_toko', ".
								"jml_min = '$jml_min', ".
								"hrg_beli = '$hrg_beli', ".
								"hrg_jual = '$hrg_jual', ".
		...
		if (empty($kode))
			...
			//jika telah isi
				...
				if (empty($nama))
					...
					mysql_query("INSERT aINTO stock(kd, kd_brg, jml_toko, jml_gudang, jml_min, hrg_beli, hrg_jual, persen) VALUES ".
...
if (empty($katkd))
	...
	{
	$qke = mysql_query("SELECT * FROM m_kategori ".
							"WHERE kd = '$katkd'");
	$rke = mysql_fetch_assoc($qke);
...
if (empty($merkkd))
	...
	{
	$qmex = mysql_query("SELECT * FROM m_merk ".
							"WHERE kd = '$merkkd'");
	$rmex = mysql_fetch_assoc($qmex);
...
//merk
$qme = mysql_query("SELECT * FROM m_merk ".
						"WHERE kd <> '$merkkd' ".
						"ORDER BY merk ASC");
$rme = mysql_fetch_assoc($qme);
...
while ($rme = mysql_fetch_assoc($qme));


echo '</select>
<br>
Kode : <br>
<input name="kode" type="text" value="'.$kode.'" size="15" maxlength="15" '.$x_enter3.'>
</td>
